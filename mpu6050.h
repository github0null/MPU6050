...
#define MPU6050_DEVICE_ADDR MPU6050_ADDRESS_AD0_LOW //IIC写入时的地址字节数据，+1为读取
...
*/
#define MPU6050_SMPLRT_DIV 0x19 //陀螺仪采样率分频，典型值：0x07(125Hz), DLPF_CFG enable: 1KHz/DIV, disable: 8KHz/DIV
...
*/
#define MPU6050_GYRO_CONFIG 0x1B //陀螺仪自检及测量范围，典型值：0x18(2000deg/s)
 ...
 * 7         | Hold        | None
*/
#define MPU6050_ACCEL_CONFIG 0x1C //加速计自检、测量范围及高通滤波频率，典型值：0x01(2G，5Hz)
...
#define MPU6050_MOT_STATUS_NEG_X 0x80 //X负轴
#define MPU6050_MOT_STATUS_POS_X 0x40 //X正轴
 ...
 * 当检测到零运动事件时，将在MOT_DETECT_STATUS寄存器(寄存器97)中显示零运动状态。
...
typedef uint8_t (*ReadByteCallBack)(uint8_t addr);
typedef void (*WriteCmdCallBack)(uint8_t regAddr, uint8_t dat);
typedef void (*DelayMsCallBk)(uint32_t ms);
...
uint8_t MPU6050_Test(DelayMsCallBk delayms);
